...
\begin{remark}[N-dimensional ball]
	When writing $\mathcal{B}(r)$ we refer to the ball of radius $r$ in $\mathbb{R}^{n}$, i.e. the set $\{x:\ \|x\| < r\}$. The closure $\overline{\mathcal{B}(r)}$ denotes the closed ball of radius $r$, i.e. the set $\{x:\ \|x\| \leq r\}$.
\end{remark}
