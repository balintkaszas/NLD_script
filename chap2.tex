...
\begin{ex}[Stability of the lower equilibrium of the pendulum]
	...
	\dot{x}_2 = -\sin(x_1).
		\end{dcases}
	\end{align}
	For small $\varepsilon>0$, this geometric procedure gives a $\delta(\varepsilon)>0$ such that the definition of stability is satisfied for $ {x}=0$. We can see in Fig. \ref{fig:pend_lower_stability} that for any initial point chosen within the blue circle, its trajectory remains within the red circle for all time (cf. Fig. \ref{fig:lyapunov_stability_def}). Therefore $ {x}=0$ is (Lyapunov) stable.
	\begin{figure}[h!]
		\centering
...
We would like to derive a more general method to analyze the stability of fixed points, thus we try to simplify our system around the fixed point and discover what this can tell us about the full (unsimplified) system. In many of the examples to follow, the system will be autonomous, but we make no such assumption here. Let us consider then the following setup
\begin{align*}
	\dot{ {x}}=f( {x,t}),\quad f\in \mathcal{C}^1,\quad  {x}=
	\begin{pmatrix}
		x_1\\ \vdots \\ x_n
...
If ${p}$ is a constant vector such that $f({p},t)={0} $, for all time $t$, then we call ${p} $ a fixed point. We can introduce the transformation ${y} = {x} - {p} $, such that the transformed system has the fixed point ${y} = {0} $ at the origin. Furthermore, we have that around ${y} = {0} $ the ODE is 
\begin{align}
	\dot{{y} } = f({p} + {y}, t) = \underbrace{f({p},t)}_{=0} + Df({p},t ){y} + o(\| {y} \|) = Df({p},t ){y} + o(\| {y} \|).
\end{align}
\begin{remark}[]
...
\begin{definition}[Linearized ODE]
	...
	\begin{align*}
		\boxed{
		\dot{{y} } = {A}(t) {y}; \quad {y} \in \mathbb{R}^{n},\quad {A}(t) := Df({p},t ) \in \mathbb{R}_{n \times n};\quad  Df(p,t) = 
	\left. \begin{pmatrix}
		\frac{\partial f_1(x,t)}{\partial x_1} & \ldots & \frac{\partial f_1(x,t)}{\partial x_n} \\
		\vdots & & \vdots \\
		\frac{\partial f_n(x,t)}{\partial x_1} & \ldots & \frac{\partial f_n(x,t)}{\partial x_n}
...
\section{Stability of $y=0$ from general estimates}
Recall the upper estimate \eqref{eq:upper_bound_on_linear_sol}, which directly applies to the linearized system \eqref{eq:sstar}. Analogously, we can derive the lower estimate
\begin{align}
\label{eq:lower_bound_on_linear_sol}
\|y_0\|e^{\int_{t_0}^t\lambda_{\text{min}}[{S}(s)]ds} \leq \|y(t) \|, \quad \text{ where } S = \frac{A^T + A}{2}.
\end{align}
Combining \eqref{eq:lower_bound_on_linear_sol} and \eqref{eq:upper_bound_on_linear_sol}, we obtain
\begin{align}
\label{eq:combined_lower_bound_on_linear_sol}
\|y_0\|e^{\int_{t_0}^t\lambda_{\text{min}}[{S}(s)]ds} \leq \|y(t) \| \leq \|y_0\|e^{\int_{t_0}^t\lambda_{\text{max}}[{S}(s)]ds},
\end{align}
with $\lambda_{min}[S(t)]$ and $\lambda_{max}[S(t)]$ denoting the time-dependent smallest and largest eigenvalues of the rate-of-strain tensor $S$. Let us consider the following cases
\begin{itemize}
\item Assume $\lambda_{max}[S(t)]<0$ for all $t\in\mathbb{R}$. In this case, $y=0$ is stable in system \eqref{eq:sstar}. We can simply select $\delta(t_0,\varepsilon) = \varepsilon$ in Definition \ref{def:stable}.
\item Assume $\lambda_{max}[S(t)]<-K<0$ for some $K>0$ for all $t\in \mathbb{R}$. Then, $y=0$ is asymptotically stable in system \eqref{eq:sstar}, since the upper estimate converges to zero as $t\to \infty$. 
\item Assume $\lambda_{min}[S(t)]>K>0$ for some $K>0$ for all $t\in\mathbb{R}$. Then, $y=0$ is unstable in system \eqref{eq:sstar}, because the lower estimate diverges as $t\to \infty$. 
\end{itemize}
...
\end{enumerate}
	...
	\end{enumerate}
\end{itemize}
\subsection{Evolution of observable functions}
...
Let us derive the evolution of equation of an observable. Differentiating the action of the Koopman operator, we get
\begin{equation}
...
\notebookbox{
\begin{notebook}
\href{https://drive.google.com/file/d/1U1r0C_cEhg6_cz2zG-a4f8cL6EbbJ56Q/view?usp=sharing}{Comparison of two-dimensional phase portraits for various eigenvalue configurations.}
\end{notebook}
}
...
\subsection{Linearized stability for discrete dynamical systems}
Consider the dynamical system given by the iterated map 
\begin{equation}\label{eq:discrete_dynsys}
x_{n+1} = f(x_n), \quad x\in \mathbb{R}^n.
\end{equation}

A fixed point $x^{0}$ satisfies 

\begin{equation}
f\left(x^0\right) = x^0. 
\end{equation}

Similarly to the continuous case, we can define the linearized system at $x^0$ by introducing the variable $y_n = x_n - x^0$. To leading order, we then have
\begin{equation}\label{eq:linearized_discrete_dynsys}
y_{n+1} = A y_n \quad \text{ with } A = Df\left(x^0\right). 
\end{equation}

The linear stability of the fixed point depends on the spectrum of A, which we denote as 
\begin{equation}
\text{spect} (A) = \{ \lambda_1, ..., \lambda_n \} \;\; \lambda_i \in \mathbb{C}. 
\end{equation}
As we did in the continuous case, we can differentiate three cases. 
\begin{itemize}
\item If $|\lambda_i|<1$, $\forall i$, then $x_n = x^0$ is asymptotically stable 
\item If $\exists j: $ for which $|\lambda_i|>1$, then  $x_n = x^0$ is unstable
\item If $|\lambda_i |\leq1$, $\forall i$, and for the eigenvalues $|\lambda_k|=1$ the algebraic and geometric multiplicities are the same, then  $x_n = x^0$ is stable
\end{itemize}

\begin{definition}[Hyperbolic fixed point in discrete systems]
	We call the fixed point  ${x} ={x^0} $ a \emph{hyperbolic fixed point} of \eqref{eq:discrete_dynsys} if each of the eigenvalues $\lambda_i$ of its linearization \eqref{eq:linearized_discrete_dynsys} satisfy
	\begin{align}
		\boxed{ |\lambda_i | \neq 1.}
	\end{align}
	Geometrically the eigenvalue configuration of a hyperbolic fixed point is shown in Fig. \ref{fig:hyperbol_ex}.
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.6\textwidth]{figures/ch2/15_2hyperbol_ex}
		\caption{The eigenvalue configuration of a hyperbolic fixed point of a discrete dynamical system, i.e. no eigenvalues are on the unit circle (red).}
		\label{fig:hyperbol_ex}
	\end{figure}
\end{definition}

\subsection{Nonlinear consequences of hyperbolicity}
...
\begin{theorem}[Sternberg]\label{thm:sternberg}
...
\notebookbox{
\begin{notebook}\label{notebook:sternberg}
\href{https://drive.google.com/file/d/1wA3m8bziiIVu5xwW8LotCE8mG5AQwRXE/view?usp=sharing}{Calculation of the linearizing transformation as described by Sternberg's theorem.}
\end{notebook}
}
...
\notebookbox{
\begin{notebook}
\href{https://drive.google.com/file/d/1i-ev2ZDkV0GgcCnUb6I6AWtSRdMToBoX/view?usp=sharing}{Analysis of the equations of motion for Watt's governor.}
\end{notebook}
}
\newpage
...
\notebookbox{
\begin{notebook}
\href{https://drive.google.com/file/d/1B94k8jgRjdWPvTOAK8tHCXcZoGW8wdPZ/view?usp=sharing}{Finding approximate linear models with DMD.} 
\end{notebook}
}
...
\begin{align}
	\left.\frac{d}{dt}x_2(t)\right|_{x_2=0} = -\frac{g}{\ell}\sin(x_1) \neq 0,\ x\in S.
\end{align}
