...
\frac{d}{dt}\left[\mathcal{K}^t g\right](x_0) = D_{x_0} \left[\mathcal{K}^t g\right](x_0)\left[DF^t(x_0)\right]^{-1}f(F^t(x_0)) = \mathcal{L}^t(x_0)\left[ \mathcal{K}^t g\right](x_0)
\end{equation}
We have introduced the linear operator on the right-hand side 
\begin{equation}
\label{eq:koopman_Ldef}
 \mathcal{L}^t : g \mapsto [Dg]\left[DF^t\right]^{-1}f\circ F^t.
 \end{equation}
 Although it is true, that $\mathcal{L}^t$ acts linearly on the function space, it is by no means a simplification. Note that the expression \eqref{eq:koopman_Ldef} explicitly contains the full flow map of the original, nonlinear system. To be precise, \eqref{eq:koopman_evol} shows that the observables evolve according to a linear, infinite-dimensional, non-autonomous functional differential equation. The solution of this problem is, in general, more difficult than solving the original set of ODEs. In addition, one must choose the underlying Banach-space appropriately as well. This can be, for example, the Sobolev space $H^1$.
