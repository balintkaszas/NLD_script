...
\begin{align}
x(t_0+nT) = \underbrace{P_{t_0} \ldots P_{t_0}}_{n  \textrm{ times} } x_0= P_{t_0}^{n}x_0.
\end{align}
...
\begin{align}
\label{eq:floquet_statement}
	\boxed{
		\Phi_{t_0}^{t} = {B(t)} e^{\Lambda(t-t_0)};\quad B(t+T) = B(t)\in \mathbb{R}^{n\times n};\quad \Lambda \in \mathbb{R}^{n\times n}  \textrm{ is constant} .
	}
\end{align}
Note, that \eqref{eq:floquet_statement} implies that $B(t_0)=\mathbb{I}$, since $B(t_0)=\Phi_{t_0}^{t_0}=\mathbb{I}$. Therefore, the Poincaré-map $P_{t_0}$ can be written as 
\begin{equation}
\label{eq:poincare_map_def_}
P_{t_0} = e^{\Lambda T}.
\end{equation}
According to \eqref{eq:poincare_map_def_}, the Floquet-multipliers are the eigenvalues of $e^{\Lambda T}$, which motivates the following definition. 
...
\end{proof}
\begin{remark}[]
	The change of variables $x = B(t)y$, with the matrix defined as in \eqref{eq:floquet_statement}, transforms the system \eqref{eq6:homo} into the form $\dot{y}=\Lambda y$,  which is an autonomous linear ODE.
	
	Carrying out the change of coordinates, we obtain that
	\begin{align}
		\dot{y} = B^{-1} (A(t)B - \dot{B})y.
	\end{align}
But
\begin{align}
	\frac{d}{dt}\left( B(t)e^{\Lambda t}\right) = ABe^{\Lambda t} \implies \dot{B} + B\Lambda  = AB.
\end{align}
Together this implies $\dot{y} = \Lambda y$.
\end{remark}
...
\end{ex}
\begin{remark}
The mathematical theory of averaging can be extended to more general nonaoutonomous systems, as long as they are slowly varying. Consider the ODE
\begin{equation}
\label{chap5:eq_averaging_general}
\dot{x} = \varepsilon f(x, t, \varepsilon), \quad x\in \mathbb{R}^n, \quad 0\leq |\varepsilon| \ll 1 
\end{equation}
and assume that 
\begin{equation}
\bar{f}^{0}(y) = \lim_{T\to \infty }\frac{1}{T}\int_{t_0}^{t_0+T}f(y, t, 0) dt \quad \text{exists and is finite.}
\end{equation}
Then the averaging principle, as defined above, applies to the system \eqref{chap5:eq_averaging_general}. Special cases, in which this condition is satisfied, include 
\begin{align} 
&f(x, \omega t, \varepsilon) \quad \text{periodic time dependence}, \\
&f(x, \omega_1 t, \omega_2t, ..., \omega_k t, \varepsilon) \quad \text{quasiperiodic time dependence}.
\end{align}
For further details, we refer to Theorem 5.1 from the textbook \cite{Verhulst2023}.
\end{remark}
...
\notebookbox{
\begin{notebook}
\href{https://drive.google.com/file/d/1CrKsKWUDazi2EGkYDVVs_V8DQnwBd8PY/view?usp=sharing}{Computation of the forced response of the Duffing equation by averaging.}
\end{notebook}
}
...
\notebookbox{
\begin{notebook}
\href{https://drive.google.com/file/d/1gam-55-G3O1Gf4PU65dYBPcPikZMGj7Y/view?usp=sharing}{Computation of the forced response of the Duffing equation via harmonic balance.}
\end{notebook}
}
...
\begin{ex}[Unforced and undamped Duffing oscillator]
	For unforced systems harmonic balance can also be used to obtain internally parametrized periodic orbits. However, the expected period is a priori unknown. Consider the following
	\begin{align}
