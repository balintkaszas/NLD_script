...
\begin{ex}[Lotka-Volterra model (1925) for predator-prey interactions]
	...
	\begin{align}\label{eq:ex86}
		\begin{dcases}
			\dot{h} = a_1 h(1- bp) \\
			\dot{p} = -a_2 p(1-ch).
		\end{dcases}
	\end{align}
	As we show in Exercise \ref{exercise_lotka_volterra}, this system has a first integral and \eqref{eq:ex86} can be derived from a Hamiltonian. The phase portrait of the model and the corresponding Hamiltonian are depicted in Fig. \ref{fig:lotka_volterra}.
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.85\textwidth]{figures/ch8/3lotka_volterra.pdf}
		\caption{Left: The phase portrait of the Lotka-Volterra model. The fixed point at $p=b, h=c$ and a few periodic orbits are indicated. Right: Contours of the Hamiltonian, which is derived in Exercise \ref{exercise_lotka_volterra}.}
		\label{fig:lotka_volterra}
	\end{figure}
\end{ex}

\begin{exercise}\label{exercise_lotka_volterra}
Consider the Lotka-Volterra model \eqref{eq:ex86}.
\begin{enumerate}
...
Now that we have seen multiple examples of Hamiltonian systems, we would like to better understand what properties they all share. The setup going forward will be as follows for $n\geq 1$
\begin{align}
