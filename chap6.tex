...
\notebookbox{
\begin{notebook}
\href{https://drive.google.com/file/d/1zO-LF8Cg_BI3ywaU50Op5n0gMXqv7vsG/view?usp=sharing}{Chaotic behavior in the Duffing equation.}
\end{notebook}
}
...
\begin{ex}[2-dimensional Rayleigh-Bénard convection]
	...
	If the Rayleigh number $Ra$ exceeds a critical value $Ra_{ \textrm{crit} }$, we have a time periodic perturbation to the velocity field. The fluid trajectories have the following equations of motion
	\begin{align}
		\begin{dcases}
...
;\quad \varepsilon>0.		
	...
	\begin{figure}[h!]
		...
		\caption{Escape time from one convection cell. The number of iterations of the Poincaré map needed for escape is given by the color scale. Here $0 < \varepsilon \ll  1$.}
...
\frac{dx}{dt}= y = \pm \sqrt{x^2-\frac{1}{2}x^4} &= F(x).
\end{align}
\end{subequations}
...
\notebookbox{
\begin{notebook}
\href{https://drive.google.com/file/d/1Uqo2L_DY1GIvxcp7jXW1izTlBfaYJL6a/view?usp=sharing}{Computation of the Melnikov function for the Duffing oscillator.}
\end{notebook}
}
...
\notebookbox{
\begin{notebook}
\href{https://drive.google.com/file/d/1_S8DpCAaG7wt54cQeKiLT8mcxDUX7UKb/view?usp=sharing}{Invariant sets of Smale's horseshoe map.}
\end{notebook}
}
...
It is possible to classify a lot of the regular (periodic) orbits of $\sigma $ in $\Sigma$. The classes and their cardinality are as follows:
\begin{enumerate}
...
\begin{align}
	\sum_{i=-\infty }^{\infty } \frac{1}{2^{|i|}} =1+ 2\sum_{i=1 }^{\infty } \frac{1}{2^{|i|}} =3.
\end{align}
